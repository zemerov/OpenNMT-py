...
def tokenize_word(
    ...
    # Add </w> token to the end of the word
    sw_tokens[-1] = sw_tokens[-1] + "</w>"

    used_per_id = np.zeros(len(merge_rules))
    missed_per_id = np.zeros(len(merge_rules))
    ...
    while len(merge_heap):
        ...
        # Apply dropout
        if dropout_table is None:
            if random_generator.rand() < dropout:
                dropped_merges.append([cur_priority, cur_pos])
                missed_per_id[cur_priority] += 1
                continue
            else:
                used_per_id[cur_priority] += 1
        else:
            if (
                random_generator.rand() < 1 - dropout_table[cur_priority]
            ):  # Apply custom dropout table
                dropped_merges.append([cur_priority, cur_pos])
                missed_per_id[cur_priority] += 1
                continue
            else:
    ...
    return sw_tokens, (used_per_id, missed_per_id)
...
def tokenize_text(
    ...
    **args
    ...
    used_ids = np.zeros(len(rules))
    missed_ids = np.zeros(len(rules))
    tokenization_words = []
    ...
    for word in words:
        ...
        current_line, (current_used_ids, current_missed_ids) = tokenize_word(
            rules,
            word,
            dropout,
            random_generator,
        ...
        tokenization_words.append(current_line)
        used_ids += current_used_ids
        missed_ids += current_missed_ids

    tokenized_string = list(itertools.chain(*tokenization_words))
    tokenized_string = [word.replace('</w>', '') for word in tokenized_string]

    return tokenized_string, (used_ids, missed_ids)
